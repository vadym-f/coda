(* registration1.ml -- register module versions whose types have one parameter *)

open Core_kernel

module type Module_decl_intf = sig
  type a [@@deriving bin_read]
...
module type Versioned_module_intf = sig
  ...
  val to_latest : 'a t -> 'a latest

  module With_version : sig
    type nonrec 'a t = {version: int; t: 'a t} [@@deriving bin_io]
  end
end

module type Version_intf = sig
  type a [@@deriving bin_io]
...
module Make (Module_decl : Module_decl_intf) = struct
  module type Versioned_with_latest_intf =
    Versioned_module_intf with type 'a latest = 'a Module_decl.latest
    ...
    List.find_map !registered
      ~f:(fun (module M : Versioned_with_latest_intf) ->
        ...
        if Int.equal version M.version then
          ...
          let pos_ref = ref 0 in
          Some (M.bin_read_t Module_decl.bin_read_a ~pos_ref buf |> M.to_latest)
        else None )
  ...
  module Register (Versioned_module : Versioned_with_latest_intf) = struct
    ...
    module With_version = struct
      type nonrec 'a t = {version: int; t: 'a Versioned_module.t}
      [@@deriving bin_io]
...
module Make_version (Version : Version_intf) = struct
  module With_version = struct
    type nonrec 'a t = {version: int; t: 'a Version.t} [@@deriving bin_io]
  ...
  let bin_read_t read_a buf ~pos_ref =
    let With_version.({version= read_version; t}) =
      With_version.bin_read_t read_a buf ~pos_ref
    in
    (* sanity check *)
  ...
  let bin_reader_t reader_a =
    let open Bin_prot.Type_class in
    let {read= read_a; vtag_read= vtag_read_a} = reader_a in
    {read= bin_read_t read_a; vtag_read= __bin_read_t__ vtag_read_a}

  let bin_size_t a_size t =
    With_version.create t |> With_version.bin_size_t a_size

  let bin_write_t write_a buf ~pos t =
    With_version.create t |> With_version.bin_write_t write_a buf ~pos

  let bin_writer_t writer_a =
    let open Bin_prot.Type_class in
    let {size= size_a; write= write_a} = writer_a in
    {size= bin_size_t size_a; write= bin_write_t write_a}

  let bin_shape_t shape_a = With_version.bin_shape_t shape_a

  let bin_t t_a =
    let open Bin_prot.Type_class in
    let {shape= shape_a; writer= writer_a; reader= reader_a} = t_a in
    { shape= bin_shape_t shape_a
    ; writer= bin_writer_t writer_a
    ; reader= bin_reader_t reader_a }
end

(* like Make_version, but for special case of latest version *)
module Make_latest_version (Version : Version_intf) = struct
  type 'a latest = 'a Version.t
