...
See r1cs_bg_split_ppzksnark.hpp .
...
// the proving and verification keys also should have a bunch of xs in, where are these?

#ifndef R1CS_BG_SPLIT_PPZKSNARK_TCC_
#define R1CS_BG_SPLIT_PPZKSNARK_TCC_
...
bool r1cs_bg_split_ppzksnark_proving_key<ppT>::operator==(const r1cs_bg_split_ppzksnark_proving_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_split_ppzksnark_proving_key<ppT> &pk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_split_ppzksnark_proving_key<ppT> &pk)
{
    in >> pk.alpha_g1;
...
/* ----------------- verification key defs -- gamma is removed ------------------ */

template<typename ppT>
bool r1cs_bg_split_ppzksnark_verification_key<ppT>::operator==(const r1cs_bg_split_ppzksnark_verification_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_split_ppzksnark_verification_key<ppT> &vk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_split_ppzksnark_verification_key<ppT> &vk)
{
...
template<typename ppT>
bool r1cs_bg_split_ppzksnark_processed_verification_key<ppT>::operator==(const r1cs_bg_split_ppzksnark_processed_verification_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_split_ppzksnark_processed_verification_key<ppT> &pvk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_split_ppzksnark_processed_verification_key<ppT> &pvk)
{
...
template<typename ppT>
bool r1cs_bg_split_ppzksnark_simex_proof<ppT>::operator==(const r1cs_bg_split_ppzksnark_simex_proof<ppT> &other) const
{
    return (this->g_A == other.g_A &&
            this->g_B == other.g_B &&
            this->g_C == other.g_C &&
            // g_z = H(A, B, C, delta_prime) . d
            this->g_z == other.g_z &&
            // delta_prime = d . delta
            this->g_delta_prime == other.g_delta_prime);
}

template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    out << simex_proof.g_A << OUTPUT_NEWLINE;
    out << simex_proof.g_B << OUTPUT_NEWLINE;
    out << simex_proof.g_C << OUTPUT_NEWLINE;
    out << simex_proof.g_z << OUTPUT_NEWLINE;
    out << simex_proof.g_delta_prime << OUTPUT_NEWLINE;

    return out;
}

template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    in >> simex_proof.g_A;
    libff::consume_OUTPUT_NEWLINE(in);
    in >> simex_proof.g_B;
...
template<typename ppT>
r1cs_bg_split_ppzksnark_verification_key<ppT> r1cs_bg_split_ppzksnark_verification_key<ppT>::dummy_verification_key(const size_t input_size)
{
    r1cs_bg_split_ppzksnark_verification_key<ppT> result;
...
template <typename ppT>
r1cs_bg_split_ppzksnark_keypair<ppT> r1cs_bg_split_ppzksnark_generator(const r1cs_bg_split_ppzksnark_constraint_system<ppT> &r1cs)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_generator");

    /* Make the B_query "lighter" if possible */
    r1cs_bg_split_ppzksnark_constraint_system<ppT> r1cs_copy(r1cs);
    r1cs_copy.swap_AB_if_beneficial();

    // elements in proving/verification keys
    ...
    At, Bt, and Ct are ui(t), vi(t), and wi(t); Ht is h(t) ? */
...
// this is changed to just ABC now, so that we don't need to compute it in the verification algorithm
// but we could just get rid

    // The ABC product component: (beta*A_i(t) + alpha*B_i(t) + C_i(t)).
    ...
    // we don't have d at this point so it stays like this and gets divided by d when constructing the proof
    ...
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_generator");

    accumulation_vector<libff::G1<ppT> > ABC_g1(std::move(ABC_g1_0), std::move(ABC_g1_values));

    r1cs_bg_split_ppzksnark_verification_key<ppT> vk = r1cs_bg_split_ppzksnark_verification_key<ppT>(alpha_g1_beta_g2,
                                                                                         //gamma_g2,
                                                                                         delta_g2,
                                                                                         ABC_g1
                                                                                         );

    r1cs_bg_split_ppzksnark_proving_key<ppT> pk = r1cs_bg_split_ppzksnark_proving_key<ppT>(std::move(alpha_g1),
                                                                               std::move(beta_g1),
                                                                               std::move(beta_g2),
                                                                               std::move(delta_g1),
                                                                               std::move(delta_g2),
    ...
    return r1cs_bg_split_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk));
}
...
template <typename ppT>
r1cs_bg_split_ppzksnark_proof<ppT> r1cs_bg_split_ppzksnark_prover(const r1cs_bg_split_ppzksnark_proving_key<ppT> &pk,
                                                      const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                                      const r1cs_bg_split_ppzksnark_auxiliary_input<ppT> &auxiliary_input,
						                              const libff::Fr<ppT> &d
)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_prover");
    ...
    // we could also generate d here if we don't feed it in with the witness & aux info
    ...
    libff::G1<ppT> delta_prime_g1 = d * pk.delta_g1;
    libff::G2<ppT> delta_prime_g2 = d * pk.delta_g2;
    const libff::Fr<ppT> delta_prime_inverse = delta_prime.inverse();

    /* A = alpha + sum_i(a_i*A_i(t)) + r*delta_prime */
    libff::G1<ppT> g1_A = pk.alpha_g1 + evaluation_At + r * delta_prime_g1;

    /* B = beta + sum_i(a_i*B_i(t)) + s*delta_prime */
    libff::G1<ppT> g1_B = pk.beta_g1 + evaluation_Bt.h + s * delta_prime_g1;
    libff::G2<ppT> g2_B = pk.beta_g2 + evaluation_Bt.g + s * delta_prime_g2;

    // divide by delta_prime instead of delta by dividing evaulation results also by d 

    /* C = sum_i(a_i*((beta*A_i(t) + alpha*B_i(t) + C_i(t)) + H(t)*Z(t))/delta_prime) 
                                + A*s + r*b - r*s*delta_prime */
    libff::G1<ppT> g1_C = delta_prime_inverse * (evaluation_Ht + evaluation_Lt)  + s *  g1_A + r * g1_B - (r * s) * delta_prime_g1;

    libff::leave_block("Compute the proof");

    libff::leave_block("Call to r1cs_bg_split_ppzksnark_prover");

    r1cs_bg_split_ppzksnark_proof<ppT> proof = r1cs_bg_split_ppzksnark_proof<ppT>(std::move(g1_A), std::move(g2_B), std::move(g1_C), std::move(delta_prime_g2));
    proof.print_size();

    return proof;
}

/* ------------------ part two of proof creation --------------------- */


template <typename ppT>
r1cs_bg_split_ppzksnark_simex_proof<ppT> r1cs_bg_split_ppzksnark_simex_prover(const r1cs_bg_split_ppzksnark_proving_key<ppT> &pk,
                                                      const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                                      const r1cs_bg_split_ppzksnark_auxiliary_input<ppT> &auxiliary_input,
                                                      const r1cs_bg_split_ppzksnark_proof<ppT> &proof,
                                                      const libff::Fr<ppT> &d,
                                                      const libff::Fr<ppT> &hash_digest
)
{
    z = hash_digest * d
    libff::G1<ppT> g1_hash_digest = hash_digest * g1_generator;
    libff::G1<ppT> g1_z = z * g1_generator;

    r1cs_bg_split_ppzksnark_simex_proof<ppT> simex_proof = r1cs_bg_split_ppzksnark_simex_proof<ppT>(std::move(g1_A), std::move(g2_B), std::move(g1_C), std::move(g1_z), std::move(delta_prime_g2));
    simex_proof.print_size();
    return simex_proof;
}
...
template <typename ppT>
r1cs_bg_split_ppzksnark_processed_verification_key<ppT> r1cs_bg_split_ppzksnark_verifier_process_vk(const r1cs_bg_split_ppzksnark_verification_key<ppT> &vk)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_verifier_process_vk");

    r1cs_bg_split_ppzksnark_processed_verification_key<ppT> pvk;
    ...
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_verifier_process_vk");

    return pvk;
}

template <typename ppT>
bool r1cs_bg_split_ppzksnark_online_verifier_weak_IC(const r1cs_bg_split_ppzksnark_processed_verification_key<ppT> &pvk,
                                               const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_online_verifier_weak_IC");
    ...
    libff::enter_block("Check if the simex_proof is well-formed");
    if (!simex_proof.is_well_formed())
    {
        if (!libff::inhibit_profiling_info)
        {
            libff::print_indent(); printf("At least one of the simex_proof elements does not lie on the curve.\n");
        }
        result = false;
    }
    libff::leave_block("Check if the simex_proof is well-formed");

    libff::enter_block("Online pairing computations");
    libff::enter_block("Check QAP divisibility");
    const libff::G1_precomp<ppT> simex_proof_g_A_precomp = ppT::precompute_G1(simex_proof.g_A);
    const libff::G2_precomp<ppT> simex_proof_g_B_precomp = ppT::precompute_G2(simex_proof.g_B);
    const libff::G1_precomp<ppT> simex_proof_g_C_precomp = ppT::precompute_G1(simex_proof.g_C);
    const libff::G1_precomp<ppT> simex_proof_g_z_precomp = ppT::precompute_G1(simex_proof.g_z);
    const libff::G1_precomp<ppT> simex_proof_g_delta_prime_precomp = ppT::precompute_G1(simex_proof.g_delta_prime);

    const libff::G1_precomp<ppT> acc_precomp = ppT::precompute_G1(acc);

    const libff::Fqk<ppT> QAP1 = ppT::miller_loop(simex_proof_g_A_precomp,  simex_proof_g_B_precomp);
    const libff::Fqk<ppT> QAP2 = ppT::double_miller_loop(
        acc_precomp, //pvk.vk_gamma_g2_precomp,
        simex_proof_g_C_precomp, pvk.vk_delta_g2_precomp);
    ...
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_online_verifier_weak_IC");

    return result;
}

template<typename ppT>
bool r1cs_bg_split_ppzksnark_verifier_weak_IC(const r1cs_bg_split_ppzksnark_verification_key<ppT> &vk,
                                        const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                        const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_verifier_weak_IC");
    r1cs_bg_split_ppzksnark_processed_verification_key<ppT> pvk = r1cs_bg_split_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_bg_split_ppzksnark_online_verifier_weak_IC<ppT>(pvk, primary_input, simex_proof);
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_verifier_weak_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_split_ppzksnark_online_verifier_strong_IC(const r1cs_bg_split_ppzksnark_processed_verification_key<ppT> &pvk,
                                                 const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                                 const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    bool result = true;
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_online_verifier_strong_IC");
    ...
    {
        result = r1cs_bg_split_ppzksnark_online_verifier_weak_IC(pvk, primary_input, simex_proof);
    }

    libff::leave_block("Call to r1cs_bg_split_ppzksnark_online_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_split_ppzksnark_verifier_strong_IC(const r1cs_bg_split_ppzksnark_verification_key<ppT> &vk,
                                          const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                          const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_verifier_strong_IC");
    r1cs_bg_split_ppzksnark_processed_verification_key<ppT> pvk = r1cs_bg_split_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_bg_split_ppzksnark_online_verifier_strong_IC<ppT>(pvk, primary_input, simex_proof);
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_split_ppzksnark_affine_verifier_weak_IC(const r1cs_bg_split_ppzksnark_verification_key<ppT> &vk,
                                               const r1cs_bg_split_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_bg_split_ppzksnark_simex_proof<ppT> &simex_proof)
{
    libff::enter_block("Call to r1cs_bg_split_ppzksnark_affine_verifier_weak_IC");
    ...
    libff::enter_block("Check QAP divisibility");
    const libff::affine_ate_G1_precomp<ppT> simex_proof_g_A_precomp = ppT::affine_ate_precompute_G1(simex_proof.g_A);
    const libff::affine_ate_G2_precomp<ppT> simex_proof_g_B_precomp = ppT::affine_ate_precompute_G2(simex_proof.g_B);
    const libff::affine_ate_G1_precomp<ppT> simex_proof_g_C_precomp = ppT::affine_ate_precompute_G1(simex_proof.g_C);
    const libff::affine_ate_G1_precomp<ppT> simex_proof_g_z_precomp = ppT::affine_ate_precompute_G1(simex_proof.g_z);
    const libff::affine_ate_G2_precomp<ppT> simex_proof_g_delta_prime_precomp = ppT::affine_ate_precompute_G2(simex_proof.g_delta_prime);
    ...
    const libff::Fqk<ppT> QAP_miller = ppT::affine_ate_e_times_e_over_e_miller_loop(
        ...
        simex_proof_g_C_precomp, pvk_vk_delta_g2_precomp,
        simex_proof_g_A_precomp,  simex_proof_g_B_precomp);
    ...
    libff::leave_block("Call to r1cs_bg_split_ppzksnark_affine_verifier_weak_IC");

    return result;
}

} // libsnark
#endif // R1CS_BG_PPZKSNARK_TCC_
