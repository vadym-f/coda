...
protoboard<FieldT>* camlsnark_mnt6_protoboard_create() {
  return new protoboard<FieldT>();
}

void camlsnark_mnt6_protoboard_delete(protoboard<FieldT>* pb) {
  delete pb;
}
...
int camlsnark_mnt6_protoboard_num_variables(protoboard<FieldT>* pb) {
  return pb->num_variables();
}

std::vector<FieldT>* camlsnark_mnt6_protoboard_auxiliary_input(protoboard<FieldT>* pb) {
  return new std::vector<FieldT>(pb->auxiliary_input());
}

void camlsnark_mnt6_protoboard_augment_variable_annotation(
...
pb_variable<FieldT>* camlsnark_mnt6_protoboard_allocate_variable(protoboard<FieldT>* pb) {
  pb_variable<FieldT>* x = new pb_variable<FieldT>();
  x->allocate(*pb, "pb_var");
  return x;
}

pb_variable_array<FieldT>* camlsnark_mnt6_protoboard_allocate_variable_array(protoboard<FieldT>* pb, int n) {
...
int camlsnark_mnt6_protoboard_variable_index(pb_variable<FieldT>* v) {
  return v->index;
}

pb_variable_array<FieldT>* camlsnark_mnt6_protoboard_variable_array_create() {
  return new pb_variable_array<FieldT>();
}

void camlsnark_mnt6_protoboard_variable_array_delete(pb_variable_array<FieldT>* arr) {
  delete arr;
}

void camlsnark_mnt6_protoboard_variable_array_emplace_back(pb_variable_array<FieldT>* arr, pb_variable<FieldT>* v) {
  arr->emplace_back(*v);
}
...
void camlsnark_mnt6_protoboard_renumber_and_append_constraints(
    protoboard<FieldT>* pb,
  ...
  for (int i = 0; i < num_source_constraints; ++i) {
    ...
    c.a = camlsnark_mnt6_linear_combination_renumber(c.a, *changes, aux_shift);
    c.b = camlsnark_mnt6_linear_combination_renumber(c.b, *changes, aux_shift);
    c.c = camlsnark_mnt6_linear_combination_renumber(c.c, *changes, aux_shift);
...
void camlsnark_mnt6_protoboard_set_variable(protoboard<FieldT>* pb, pb_variable<FieldT>* x, FieldT* y) {
  pb->val(*x) = *y;
}

FieldT* camlsnark_mnt6_protoboard_get_variable(protoboard<FieldT>* pb, pb_variable<FieldT>* x) {
  return new FieldT(pb->val(*x));
}

void camlsnark_mnt6_init_public_params() {
  ppT::init_public_params();
}

int camlsnark_mnt6_field_size_in_bits() {
  auto n = FieldT::size_in_bits();
  return n;
}

libff::bigint<libff::mnt6_r_limbs>* camlsnark_mnt6_field_size() {
  libff::bigint<libff::mnt6_r_limbs>* x = new libff::bigint<libff::mnt6_r_limbs>(FieldT::field_char());
  return x;
}

variable<FieldT>* camlsnark_mnt6_var_create(int i) {
...
size_t camlsnark_mnt6_var_index(variable<FieldT>* v) {
  return v->index;
}

bool camlsnark_mnt6_field_is_square(FieldT* x) {
  FieldT y = *x ^ FieldT::euler;
  return y == FieldT::one();
}

FieldT* camlsnark_mnt6_field_sqrt(FieldT* x) {
  return new FieldT(x->sqrt());
}

FieldT* camlsnark_mnt6_field_random() {
  return new FieldT(FieldT::random_element());
}

FieldT* camlsnark_mnt6_field_of_int(int n) {
  return new FieldT(n);
}

FieldT* camlsnark_mnt6_field_inv(FieldT* x) {
  return new FieldT(x->inverse());
}

FieldT* camlsnark_mnt6_field_square(FieldT* x) {
  return new FieldT(x->squared());
}

FieldT* camlsnark_mnt6_field_add(FieldT* x, FieldT* y) {
  return new FieldT(*x + *y);
}

FieldT* camlsnark_mnt6_field_mul(FieldT* x, FieldT* y) {
  return new FieldT(*x * *y);
}

FieldT* camlsnark_mnt6_field_sub(FieldT* x, FieldT* y) {
  return new FieldT(*x - *y);
}

FieldT* camlsnark_mnt6_field_rng(int i) {
  return new FieldT(libff::SHA512_rng<FieldT>(i));
}

void camlsnark_mnt6_field_delete(FieldT* f) {
  delete f;
}

void camlsnark_mnt6_field_print(FieldT* f) {
  f->print();
}

// bigint mnt6_r
libff::bigint<libff::mnt6_r_limbs>* camlsnark_mnt6_bigint_r_of_field(FieldT* x) {
  return new libff::bigint<libff::mnt6_r_limbs>(x->as_bigint());
}
...
libff::bigint<libff::mnt6_r_limbs>* camlsnark_mnt6_bigint_r_div(
  ...
  return new libff::bigint<libff::mnt6_r_limbs>(q);
}

FieldT* camlsnark_mnt6_bigint_r_to_field(libff::bigint<libff::mnt6_r_limbs>* n) {
  return new FieldT(*n);
}

libff::bigint<libff::mnt6_r_limbs>* camlsnark_mnt6_bigint_r_of_numeral(const unsigned char* s, int s_length, int base) {
  libff::bigint<libff::mnt6_r_limbs>* res = new libff::bigint<libff::mnt6_r_limbs>();

  assert (base >= 2 && base <= 256);

  mp_size_t limbs_written = mpn_set_str(res->data, s, s_length, base);
  assert(limbs_written <= libff::mnt6_r_limbs);

  return res;
}

int camlsnark_mnt6_bigint_r_compare(
    libff::bigint<libff::mnt6_r_limbs>* n1,
    libff::bigint<libff::mnt6_r_limbs>* n2) {
  return mpn_cmp(n1->data, n2->data, libff::mnt6_r_limbs);
}

bool camlsnark_mnt6_bigint_r_test_bit(libff::bigint<libff::mnt6_r_limbs>* n, int i) {
  return n->test_bit(i);
}

void camlsnark_mnt6_bigint_r_delete(libff::bigint<libff::mnt6_r_limbs>* n) {
  delete n;
}

void camlsnark_mnt6_bigint_r_print(libff::bigint<libff::mnt6_r_limbs>* n) {
  n->print();
}

std::vector<long>* camlsnark_mnt6_bigint_r_find_wnaf(
    size_t window_size, libff::bigint<libff::mnt6_r_limbs>* scalar) {
  return new std::vector<long>(libff::find_wnaf(window_size, *scalar));
}

// bigint mnt6_q
bool camlsnark_mnt6_bigint_q_test_bit(libff::bigint<libff::mnt6_q_limbs>* n, int i) {
  return n->test_bit(i);
}

void camlsnark_mnt6_bigint_q_delete(libff::bigint<libff::mnt6_q_limbs>* n) {
  delete n;
}

void camlsnark_mnt6_bigint_q_print(libff::bigint<libff::mnt6_q_limbs>* n) {
  n->print();
}

std::vector<long>* camlsnark_mnt6_bigint_q_find_wnaf(
    size_t window_size, libff::bigint<libff::mnt6_q_limbs>* scalar) {
  return new std::vector<long>(libff::find_wnaf(window_size, *scalar));
}

bool camlsnark_mnt6_field_equal(FieldT* x1, FieldT* x2) {
  return *x1 == *x2;
}

// begin linear_combination_vector
std::vector<linear_combination<FieldT>>* camlsnark_mnt6_linear_combination_vector_create() {
  return new std::vector<linear_combination<FieldT>>();
}

void camlsnark_mnt6_linear_combination_vector_delete(std::vector<linear_combination<FieldT>>* v) {
  delete v;
}

int camlsnark_mnt6_linear_combination_vector_length(std::vector<linear_combination<FieldT>> *v) {
  return v->size();
}

void camlsnark_mnt6_linear_combination_vector_emplace_back(std::vector<linear_combination<FieldT>>* v, linear_combination<FieldT>* x) {
  v->emplace_back(*x);
}

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_vector_get(std::vector<linear_combination<FieldT>>* v, int i) {
  linear_combination<FieldT> res = (*v)[i];
  return new linear_combination<FieldT>(res);
}
// end linear_combination_vector

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_create() {
  return new linear_combination<FieldT>();
}

void camlsnark_mnt6_linear_combination_add_term(linear_combination<FieldT>* lc, FieldT* coeff, variable<FieldT>* v) {
  lc->add_term(*v, *coeff);
}

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_of_var(variable<FieldT>* v) {
  return new linear_combination<FieldT>(*v);
}

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_of_int(int n) {
  return new linear_combination<FieldT>(n);
}

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_of_field(FieldT* x) {
  return new linear_combination<FieldT>(*x);
}

void camlsnark_mnt6_linear_combination_delete(linear_combination<FieldT>* lc) {
  delete lc;
}

void camlsnark_mnt6_linear_combination_print(linear_combination<FieldT>* lc) {
  lc->print();
}
...
linear_combination<FieldT>* camlsnark_mnt6_linear_combination_var_sub(variable<FieldT>* v, linear_combination<FieldT>* other) {
  auto neg = -(*other);
  return camlsnark_mnt6_linear_combination_var_add(v, &neg);
}

linear_combination<FieldT>* camlsnark_mnt6_linear_combination_of_terms(std::vector<linear_term<FieldT>>* v) {
  return new linear_combination<FieldT>(*v);
}

linear_term<FieldT>* camlsnark_mnt6_linear_combination_term_create(FieldT* x, variable<FieldT>* v) {
  return new linear_term<FieldT>(*v, *x);
}

void camlsnark_mnt6_linear_combination_term_delete(linear_term<FieldT>* lt) {
  delete lt;
}
...
void camlsnark_mnt6_linear_combination_term_vector_delete(std::vector<linear_term<FieldT>>* v) {
  delete v;
}

int camlsnark_mnt6_linear_combination_term_vector_length(std::vector<linear_term<FieldT>> *v) {
  return v->size();
}

// Not too sure what's going on here memory-wise...
void camlsnark_mnt6_linear_combination_term_vector_emplace_back(std::vector<linear_term<FieldT>>* v, linear_term<FieldT>* x) {
  v->emplace_back(*x);
}
...
void camlsnark_mnt6_linear_combination_update_digest(
    linear_combination<FieldT>& lc,
    MD5_CTX* ctx) {
  long coeff_size_in_bytes = libff::mnt6_r_limbs * sizeof(mp_limb_t);
...
std::string* camlsnark_mnt6_r1cs_constraint_system_digest(
  ...
  for (size_t i = 0; i < cs.size(); ++i) {
    ...
    camlsnark_mnt6_linear_combination_update_digest(c.a, &ctx);
    camlsnark_mnt6_linear_combination_update_digest(c.b, &ctx);
    camlsnark_mnt6_linear_combination_update_digest(c.c, &ctx);
  }
...
bool camlsnark_mnt6_r1cs_constraint_system_is_satisfied(
    r1cs_constraint_system<FieldT>* sys,
...
bool camlsnark_mnt6_linear_combination_check(
...
bool camlsnark_mnt6_r1cs_constraint_system_check(r1cs_constraint_system<FieldT>* sys) {
  std::vector<r1cs_constraint<FieldT>>& cs = sys->constraints;
  size_t total_input_size = sys->num_variables();
  for (size_t i = 0; i < cs.size(); ++i) {
    r1cs_constraint<FieldT> c = cs[i];
    if (!camlsnark_mnt6_linear_combination_check(total_input_size, c.a)) { return false; }
    if (!camlsnark_mnt6_linear_combination_check(total_input_size, c.b)) { return false; }
    if (!camlsnark_mnt6_linear_combination_check(total_input_size, c.c)) { return false; }
  }
  return true;
}
...
void camlsnark_mnt6_r1cs_constraint_system_report_statistics(r1cs_constraint_system<FieldT>* sys) {
...
void camlsnark_mnt6_r1cs_constraint_system_add_constraint_with_annotation(
    r1cs_constraint_system<FieldT>* sys, 
    r1cs_constraint<FieldT>* c,
    char* s) {
  std::string str(s);
...
int camlsnark_mnt6_r1cs_constraint_system_get_primary_input_size(
    r1cs_constraint_system<FieldT>* sys) {
  return sys->primary_input_size;
}

int camlsnark_mnt6_r1cs_constraint_system_get_auxiliary_input_size(
    r1cs_constraint_system<FieldT>* sys) {
...
std::vector<libff::bigint<libff::mnt6_r_limbs>>* camlsnark_mnt6_bigint_r_vector_create() {
  return new std::vector<libff::bigint<libff::mnt6_r_limbs>>();
}

int camlsnark_mnt6_bigint_r_vector_length(std::vector<libff::bigint<libff::mnt6_r_limbs>> *v) {
  return v->size();
}

// Not too sure what's going on here memory-wise...
void camlsnark_mnt6_bigint_r_vector_emplace_back(std::vector<libff::bigint<libff::mnt6_r_limbs>>* v, libff::bigint<libff::mnt6_r_limbs>* x) {
  v->emplace_back(*x);
}

libff::bigint<libff::mnt6_r_limbs>* camlsnark_mnt6_bigint_r_vector_get(std::vector<libff::bigint<libff::mnt6_r_limbs>>* v, int i) {
  libff::bigint<libff::mnt6_q_limbs> res = (*v)[i];
  return new libff::bigint<libff::mnt6_r_limbs>(res);
}

void camlsnark_mnt6_bigint_r_vector_delete(std::vector<libff::bigint<libff::mnt6_q_limbs>>* v) {
  delete v;
}


// vectors of bigint_q elements
std::vector<libff::bigint<libff::mnt6_q_limbs>>* camlsnark_mnt6_bigint_q_vector_create() {
  return new std::vector<libff::bigint<libff::mnt6_q_limbs>>();
}

int camlsnark_mnt6_bigint_q_vector_length(std::vector<libff::bigint<libff::mnt6_q_limbs>> *v) {
  return v->size();
}

// Not too sure what's going on here memory-wise...
void camlsnark_mnt6_bigint_q_vector_emplace_back(std::vector<libff::bigint<libff::mnt6_q_limbs>>* v, libff::bigint<libff::mnt6_q_limbs>* x) {
  v->emplace_back(*x);
}

libff::bigint<libff::mnt6_q_limbs>* camlsnark_mnt6_bigint_q_vector_get(std::vector<libff::bigint<libff::mnt6_q_limbs>>* v, int i) {
  libff::bigint<libff::mnt6_q_limbs> res = (*v)[i];
  return new libff::bigint<libff::mnt6_q_limbs>(res);
}

void camlsnark_mnt6_bigint_q_vector_delete(std::vector<libff::bigint<libff::mnt6_q_limbs>>* v) {
  delete v;
}
...
r1cs_bg_ppzksnark_proving_key<ppT>* camlsnark_mnt6_bg_keypair_pk(r1cs_bg_ppzksnark_keypair<ppT>* keypair) {
  return new r1cs_bg_ppzksnark_proving_key<ppT>(keypair->pk);
}

r1cs_bg_ppzksnark_verification_key<ppT>* camlsnark_mnt6_bg_keypair_vk(r1cs_bg_ppzksnark_keypair<ppT>* keypair) {
  return new r1cs_bg_ppzksnark_verification_key<ppT>(keypair->vk);
}

void camlsnark_mnt6_bg_keypair_delete(r1cs_bg_ppzksnark_keypair<ppT>* keypair) {
  delete keypair;
}

void camlsnark_mnt6_bg_proving_key_delete(r1cs_bg_ppzksnark_proving_key<ppT>* pk) {
  delete pk;
}

std::string* camlsnark_mnt6_bg_proving_key_to_string(r1cs_bg_ppzksnark_proving_key<ppT>* pk) {
  std::stringstream stream;
  stream << *pk;
  return new std::string(stream.str());
}

r1cs_bg_ppzksnark_proving_key<ppT>* camlsnark_mnt6_bg_proving_key_of_string(std::string* s) {
...
void camlsnark_mnt6_bg_verification_key_delete(r1cs_bg_ppzksnark_verification_key<ppT>* vk) {
  delete vk;
}

int camlsnark_mnt6_bg_verification_key_size_in_bits(
    r1cs_bg_ppzksnark_verification_key<ppT>* vk
) {
  return vk->size_in_bits();
}

std::string* camlsnark_mnt6_bg_verification_key_to_string(r1cs_bg_ppzksnark_verification_key<ppT>* vk) {
  std::stringstream stream;
  stream << *vk;
  return new std::string(stream.str());
}

r1cs_bg_ppzksnark_verification_key<ppT>* camlsnark_mnt6_bg_verification_key_of_string(std::string* s) {
...
libff::G1<ppT>* camlsnark_mnt6_bg_proof_get_a(r1cs_bg_ppzksnark_proof<ppT>* p) {
  auto a = p->g_A;
  return new libff::G1<ppT>(a);
}

libff::G2<ppT>* camlsnark_mnt6_bg_proof_get_b(r1cs_bg_ppzksnark_proof<ppT>* p) {
  auto b = p->g_B;
  return new libff::G2<ppT>(b);
}

libff::G1<ppT>* camlsnark_mnt6_bg_proof_get_c(r1cs_bg_ppzksnark_proof<ppT>* p) {
    auto c = p->g_C;
  return new libff::G1<ppT>(c);
}

libff::G2<ppT>* camlsnark_mnt6_bg_proof_get_delta_prime(r1cs_bg_ppzksnark_proof<ppT>* p) {
