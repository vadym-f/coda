...
#ifndef R1CS_BG_PPZKSNARK_TCC_
#define R1CS_BG_PPZKSNARK_TCC_
...
bool r1cs_bg_ppzksnark_proving_key<ppT>::operator==(const r1cs_bg_ppzksnark_proving_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_ppzksnark_proving_key<ppT> &pk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_ppzksnark_proving_key<ppT> &pk)
{
    in >> pk.alpha_g1;
...
template<typename ppT>
bool r1cs_bg_ppzksnark_verification_key<ppT>::operator==(const r1cs_bg_ppzksnark_verification_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_ppzksnark_verification_key<ppT> &vk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_ppzksnark_verification_key<ppT> &vk)
{
...
template<typename ppT>
bool r1cs_bg_ppzksnark_processed_verification_key<ppT>::operator==(const r1cs_bg_ppzksnark_processed_verification_key<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_ppzksnark_processed_verification_key<ppT> &pvk)
{
...
template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_ppzksnark_processed_verification_key<ppT> &pvk)
{
...
template<typename ppT>
bool r1cs_bg_ppzksnark_proof<ppT>::operator==(const r1cs_bg_ppzksnark_proof<ppT> &other) const
{
...
template<typename ppT>
std::ostream& operator<<(std::ostream &out, const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    out << proof.g_A << OUTPUT_NEWLINE;
    out << proof.g_B << OUTPUT_NEWLINE;
    out << proof.g_C << OUTPUT_NEWLINE;
    //out << proof.g_z << OUTPUT_NEWLINE;
    out << proof.g_delta_prime << OUTPUT_NEWLINE;

    return out;
}

template<typename ppT>
std::istream& operator>>(std::istream &in, r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    in >> proof.g_A;
    libff::consume_OUTPUT_NEWLINE(in);
    in >> proof.g_B;
...
template<typename ppT>
r1cs_bg_ppzksnark_verification_key<ppT> r1cs_bg_ppzksnark_verification_key<ppT>::dummy_verification_key(const size_t input_size)
{
    r1cs_bg_ppzksnark_verification_key<ppT> result;
...
template <typename ppT>
r1cs_bg_ppzksnark_keypair<ppT> r1cs_bg_ppzksnark_generator(const r1cs_bg_ppzksnark_constraint_system<ppT> &r1cs)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_generator");

    /* Make the B_query "lighter" if possible */
    r1cs_bg_ppzksnark_constraint_system<ppT> r1cs_copy(r1cs);
    ...
    /* The 'gamma inverse 'product component: (beta*A_i(t) + alpha*B_i(t) + C_i(t)). 
	This could also be removed entirely and computed by the verifier themselves */
	...
	We don't have d at this point so it stays like this for crs and gets divided by d when constructing the proof */
    ...
    libff::leave_block("Call to r1cs_bg_ppzksnark_generator");

    accumulation_vector<libff::G1<ppT> > ABC_g1(std::move(ABC_g1_0), std::move(ABC_g1_values));

    r1cs_bg_ppzksnark_verification_key<ppT> vk = r1cs_bg_ppzksnark_verification_key<ppT>(alpha_g1_beta_g2,
                                                                                         delta_g2,
                                                                                         ABC_g1);

    r1cs_bg_ppzksnark_proving_key<ppT> pk = r1cs_bg_ppzksnark_proving_key<ppT>(std::move(alpha_g1),
                                                                               std::move(beta_g1),
                                                                               std::move(beta_g2),
                                                                               std::move(delta_g1),
                                                                               std::move(delta_g2),
    ...
    return r1cs_bg_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk));
}


/* ------------ prover -------------- */
template <typename ppT>
r1cs_bg_ppzksnark_proof<ppT> r1cs_bg_ppzksnark_prover(const r1cs_bg_ppzksnark_proving_key<ppT> &pk,
                                                      const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                                      const r1cs_bg_ppzksnark_auxiliary_input<ppT> &auxiliary_input,
						                              const libff::Fr<ppT> &d
)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_prover");
    ...
    libff::Fr<ppT> d_inverse = d.inverse();
    ...
    /* C = sum_i(a_i*((beta*A_i(t) + alpha*B_i(t) + C_i(t)) + H(t)*Z(t))/delta_prime) + A*s + r*b - r*s*delta_prime */
    libff::G1<ppT> g1_C = d_inverse * (evaluation_Ht + evaluation_Lt)  +  s * g1_A  +  r * g1_B  -  (r * s) * delta_prime_g1;

    libff::leave_block("Compute the proof");

    libff::leave_block("Call to r1cs_bg_ppzksnark_prover");

    r1cs_bg_ppzksnark_proof<ppT> proof = r1cs_bg_ppzksnark_proof<ppT>(std::move(g1_A), std::move(g2_B), std::move(g1_C), std::move(delta_prime_g2));
    proof.print_size();
...
template <typename ppT>
r1cs_bg_ppzksnark_processed_verification_key<ppT> r1cs_bg_ppzksnark_verifier_process_vk(const r1cs_bg_ppzksnark_verification_key<ppT> &vk)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_verifier_process_vk");

    r1cs_bg_ppzksnark_processed_verification_key<ppT> pvk;
    pvk.vk_alpha_g1_beta_g2 = vk.alpha_g1_beta_g2;
    pvk.vk_delta_g2_precomp = ppT::precompute_G2(vk.delta_g2);
    pvk.ABC_g1 = vk.ABC_g1;

    libff::leave_block("Call to r1cs_bg_ppzksnark_verifier_process_vk");

    return pvk;
}

template <typename ppT>
bool r1cs_bg_ppzksnark_online_verifier_weak_IC(const r1cs_bg_ppzksnark_processed_verification_key<ppT> &pvk,
                                               const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_online_verifier_weak_IC");
    ...
    //const libff::G1_precomp<ppT> proof_g_z_precomp = ppT::precompute_G1(proof.g_z);
    const libff::G2_precomp<ppT> proof_g_delta_prime_precomp = ppT::precompute_G2(proof.g_delta_prime);
...
*/
    ...
    libff::leave_block("Call to r1cs_bg_ppzksnark_online_verifier_weak_IC");

    return result;
}

template<typename ppT>
bool r1cs_bg_ppzksnark_verifier_weak_IC(const r1cs_bg_ppzksnark_verification_key<ppT> &vk,
                                        const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                        const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_verifier_weak_IC");
    r1cs_bg_ppzksnark_processed_verification_key<ppT> pvk = r1cs_bg_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_bg_ppzksnark_online_verifier_weak_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_bg_ppzksnark_verifier_weak_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_ppzksnark_online_verifier_strong_IC(const r1cs_bg_ppzksnark_processed_verification_key<ppT> &pvk,
                                                 const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                                 const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    bool result = true;
    libff::enter_block("Call to r1cs_bg_ppzksnark_online_verifier_strong_IC");
    ...
    {
        result = r1cs_bg_ppzksnark_online_verifier_weak_IC(pvk, primary_input, proof);
    }

    libff::leave_block("Call to r1cs_bg_ppzksnark_online_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_ppzksnark_verifier_strong_IC(const r1cs_bg_ppzksnark_verification_key<ppT> &vk,
                                          const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                          const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_verifier_strong_IC");
    r1cs_bg_ppzksnark_processed_verification_key<ppT> pvk = r1cs_bg_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_bg_ppzksnark_online_verifier_strong_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_bg_ppzksnark_verifier_strong_IC");
    return result;
}

template<typename ppT>
bool r1cs_bg_ppzksnark_affine_verifier_weak_IC(const r1cs_bg_ppzksnark_verification_key<ppT> &vk,
                                               const r1cs_bg_ppzksnark_primary_input<ppT> &primary_input,
                                               const r1cs_bg_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_bg_ppzksnark_affine_verifier_weak_IC");
    ...
    libff::leave_block("Call to r1cs_bg_ppzksnark_affine_verifier_weak_IC");
